...
def parse_repo(repo, item):
    if repo in item:
        return item[repo].replace('api.github.com/repos', 'github.com')
    else:
        # here should check null
        return ""
...
def parse_info(data, host):
    if host == GITHUB_HOST:
        ...
        tmp.append(reformat_str(parse_repo("repository_url", data)))
        tmp.append(reformat_str(parse_key("html_url", data)))
...
if __name__ == '__main__':
    ...
    url_list = url_repo.get_url_list(github=True, gitlab=False)
    for repo_url in url_list:
        std_repo_name = url_repo.std_table_name(repo_url)
        std_repo_name = reformat_sql_name(std_repo_name)
        ...
        while True:
            ...
            try:
                ...
                if try_time >= 5:
                    ...
                    try_time += 1
                    logger.warning("exception happened, " + str(try_time) + " times try...")
                    logger.error(e)
                    continue
        db_driver.db_close()
